...
function navigate(view) {
  ...
  document.getElementById('addTxBtn').classList.toggle('hidden', view !== 'transactions');
  renderView(view);
}
...
/* ---------------- MONTH/YEAR PICKER ---------------- */
let monthPickerTarget = null; // 'dashboard' | 'budget'
let monthPickerYear = null;
const MONTH_NAMES = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];

function openMonthPicker(target) {
  monthPickerTarget = target;
  const current = target === 'dashboard' ? state.currentMonth : state.budgetMonth;
  monthPickerYear = current.y;
  renderMonthPickerGrid();
  openSheet('monthPickerModal');
}

function renderMonthPickerGrid() {
  document.getElementById('mpYearLabel').textContent = monthPickerYear;
  const current = monthPickerTarget === 'dashboard' ? state.currentMonth : state.budgetMonth;
  const grid = document.getElementById('monthPickerGrid');
  grid.innerHTML = MONTH_NAMES.map((name, i) => {
    const active = monthPickerYear === current.y && i === current.m;
    return `<button data-month="${i}" class="${active ? 'active' : ''}">${name}</button>`;
  }).join('');
  grid.querySelectorAll('button').forEach(btn => {
    btn.addEventListener('click', () => {
      const m = parseInt(btn.dataset.month, 10);
      const newMonth = { y: monthPickerYear, m };
      if (monthPickerTarget === 'dashboard') { state.currentMonth = newMonth; renderDashboard(); }
      else { state.budgetMonth = newMonth; renderBudgets(); }
      closeSheet('monthPickerModal');
    });
  });
}

document.getElementById('mpYearPrev').addEventListener('click', () => { monthPickerYear--; renderMonthPickerGrid(); });
document.getElementById('mpYearNext').addEventListener('click', () => { monthPickerYear++; renderMonthPickerGrid(); });
document.getElementById('monthLabel').addEventListener('click', () => openMonthPicker('dashboard'));
document.getElementById('budgetMonthLabel').addEventListener('click', () => openMonthPicker('budget'));
