...
function findDuplicateGroups() {
  ...
  for (const t of state.transactions) {
    const key = `${t.accountId}|${t.date}|${t.amount.toFixed(2)}|${t.isIncome ? 'in' : 'out'}`;
  ...
  wrap.innerHTML = groups.map((group, gi) => {
    ...
    return `
      <div class="dup-group" data-group="${gi}">
        <div class="dup-group-head">${group.length} matches — ${formatDate(first.date)} · ${state.currency}${first.amount.toLocaleString(undefined,{minimumFractionDigits:2})} · ${acc ? acc.name : ''} · ${first.isIncome ? 'credit' : 'debit'}</div>
        ${itemsHTML}
