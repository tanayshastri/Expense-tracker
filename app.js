...
/* ---------------- NAVIGATION ---------------- */
const VIEW_TITLES = { dashboard: 'Dashboard', trends: 'Trends', transactions: 'Activity', accounts: 'Accounts', settings: 'Settings' };
...
function renderAccountsView() {
  ...
  state.categories.forEach(c => {
    const total = state.transactions.filter(t => t.categoryId === c.id && !t.isIncome).reduce((s,t)=>s+t.amount,0);
    const row = document.createElement('div');
    row.className = 'rank-row';
    row.innerHTML = `
      <div class="rank-left"><span class="swatch" style="background:${c.color}"></span><span class="rank-name">${escapeHtml(c.name)}</span></div>
      <span class="rank-value">${state.currency}${total.toLocaleString(undefined,{maximumFractionDigits:0})}</span>`;
...
/* Prompt modal for simple add-name (+color) flows */
function openPrompt({ title, label, withColor, onSave }) {
